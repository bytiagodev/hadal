...
**Contrast.** The background travels from bright cyan at the surface to near-black in the trench, so no fixed text colour survives the full ramp. Interpolating the text colour with depth does not help either, because every colour ramp has a crossing point where the text and the water match. The fix was to stop putting text on the water at all. Every readout, zone marker, and creature caption sits on an opaque instrument housing at `#0a0f11`. This holds a 10.9:1 contrast ratio for the amber text and 16:1 for the body ink at every depth, measured, with nothing to tune.
